...
namespace TeachPanel.Api.Middleware.Filters;
...
internal sealed class ExceptionFilter : ExceptionFilterAttribute
{
	...
	private void HandleExceptionAsync(ExceptionContext context)
	{
		switch (context.Exception)
		{
			...
			case DbUpdateConcurrencyException dbUpdateConcurrencyException:
				_logger.LogWarning(dbUpdateConcurrencyException, "Concurrency exception occured during request");
				SetExceptionResult(context, StatusCodes.Status409Conflict);
				break;
			default:
	...
	private void SetExceptionResult(ExceptionContext context, int code)
	{
		...
		if (exception is CoreException coreException)
		{
			...
			responseModel.AddDetails(coreException.Extensions);
		}
		else
		{
		...
		if (code >= 500)
		{
			var hasCorrelation = context.HttpContext.Response.Headers.TryGetValue("X-Correlation-ID", out var values);
			if (hasCorrelation)
			{
				responseModel.AddDetail("CorrelationId", values.FirstOrDefault());
				responseModel.AddDetail("Tip", $"Please send this code {values.FirstOrDefault()} to the support team or try again later.");
			}
		}
